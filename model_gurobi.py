...
print("\n" + "="*80)
print("CELL 2: LOAD DATA FROM CSV FILES")
print("="*80)
...
def load_nodes(node_file):
    """
    Node file columns:
    - Capacity(PCU/year): "5000" or "5000;8000;10000" or "0;5000;8000;10000"
    - Inverst: "100;200" or "100;200;210"
    - Project: "Upgrade" or "New" or "E" or "N"
    """
    ...
    node_capacity_pcu_levels = {}   # nid -> [base, lvl1, lvl2, ...] or [single]
        ...
        with open(node_file, newline='', encoding='utf-8', errors="ignore") as f:
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                    ...
                    # Special handling for new hub (level 0 = 0)
...
# ============================================================
# 2.4 LOAD - SUPPORT READING FROM PKL
# ============================================================
...
if Path(data_pkl).exists():
    ...
    print("  ✓ Read from pkl complete!")
else:
    print(f"\n  Pkl file not found: {data_pkl}")
    ...
    # Save to pkl for next run
    print(f"\n  Saving to pkl: {data_pkl}...")
...
# Find all hubs from data
H = existing_hubs + potential_hubs
H_tilde = potential_hubs
H0 = existing_hubs
new_hubs = candidate_hubs_new
print(f"  • All hubs from data: {H}")

print(f"  • Real nodes: {len(N)} nodes")

# ============================================================
# CACHE: Save data per Data Model _ 2026-Feb_05-Eps_02.ipynb
# Save for next run - load from cache if valid
# ============================================================
_model_cache_path = Path('data/model_cache_eps02.pkl')
def _model_cache_source_hash():
    """Hash to check if cache is valid (based on source data)."""
...
# Create mapping from (u,v,mode) to arc types
def create_arc_structure(edges_raw, all_hubs_from_data):
    ...
    for edge in edges_raw:
        ...
        # Real arc: from physical node to virtual node
        real_arcs.append((u, v_virtual))
        real_arcs.append((v, u_virtual))
        
        # Virtual arc: from virtual node back to physical node
...
for arc in A:
    if arc in arc_project_type:
        ...
        # Virtual arcs and through-hub arcs
...
# ============================================
# ============================================
# CELL 3: BUILD EXTENDED GRAPH (from 2026_ToyExample.ipynb)
# ============================================
...
print(f"Extended graph: {len(G_exp)} keys in adjacency")
print(f" - Real nodes: {len(real_nodes_list)}")
print(f" - Virtual nodes: {len(virtual_nodes_list)} (format (v,mode))")

# Display portion of graph for verification
...
def build_arcs(G_exp):
    ...
    for u, out_list in G_exp.items():
        for item in out_list:
            # item can be: (v, dist, arc_type)  or (v, 0, 'virtual_arc')
            v, _, arc_type = item

            if arc_type in ("road", "water"):
                # real arcs: u must be physical node (int), v usually virtual node (tuple)
                if isinstance(u, int):
                    real_arcs.append((u, to_virtual_label(v)))

            elif arc_type == "virtual_arc":
                # virtual arcs: u usually virtual node (tuple), v usually physical node (int)
...
def add_reverse_arcs(A_tilde):
    ...
    # giữ thứ tự “đẹp”: real->virtual trước, then virtual->virtual
    def arc_key(a):
        u, v = a
...
# ============================================
# CELL 4: COMPUTE L_min FOR EACH O-D (from 2026_ToyExample.ipynb)
# ============================================
...
# Check if pkl file exists (to skip CELL 4 if needed)
...
if Path(paths_pkl_check).exists():
    try:
        ...
        if 'Lmin_dict' in paths_data_check:
            ...
            print(f"\n✓ Found Lmin_dict in pkl, skip CELL 4")
    except Exception as e:
        print(f"  ⚠️ Cannot read pkl: {e}")
...
if Lmin_dict_from_pkl is None:
    ...
    #   targets_of_d = {(d,0),(d,1),(d,2),d}  (your choice)
    # ------------------------------------------------------------
    def origin_states(o):
    ...
    def dest_targets(d):
        # destination can reach node d or states (d,1)/(d,2)
        # (d,0) in sample is actually "d" (physical). Keep both d.
        return [d, (d, 1), (d, 2)]
    ...
    print(f"\n✓ Have Lmin_dict from pkl, skip CELL 4")

# ============================================
# CELL 5: COMPUTE NEAR-OPTIMAL FOR EACH O-D (from 2026_ToyExample.ipynb)
# SUPPORT READING FROM PKL IF EXISTS
# ============================================
...
if Path(paths_pkl).exists():
    ...
    print("  ✓ Read paths from pkl complete!")
    print(f"  ✓ Number of paths: {sum(len(p) for p in paths.values())}")
    
    # Skip path computation, go to next section
    skip_path_calculation = True

EPSILON = 0.25   # Match sample notebook (Data Model _ 2026-Feb_05_eps_05.ipynb)
...
if not skip_path_calculation:
    ...
    for comm, pairs in OD_pairs.items():
        ...
        for o, d in pairs:
            ...
            # if origin has no outgoing in expanded graph, skip
    ...
    def generate_all_unique_paths_with_through_hubs(paths_list, H):
        ...
        for base_path in paths_list:
            ...
            # --- generate all non-overlapping block combinations ---
    ...
    # Save paths to pkl after CELL 5
    print("\n  Saving paths to pkl...")
    ...
    print(f"  ✓ Saved paths to: {paths_pkl}")


# Check cache L_h, L_a, w_gk (per Eps_02) - load if valid
...
if _model_cache_path.exists():
    try:
        ...
        if _cache.get('source_hash') == _model_cache_source_hash():
            print(f"\n✓ Found valid cache: {_model_cache_path}")
            print("  Loading L_h, L_a, w_gk from cache (no recompute)...")
            _use_model_cache = True
    except Exception as e:
        print(f"  ⚠️ Cannot read cache: {e}")

# Hub capacity levels 
def build_L_h(node_capacity_pcu_levels, new_hubs, H_tilde, H0):
    """
    node_capacity_pcu_levels: {node: [cap0, cap1, ...]}
    new_hubs: list of new hubs (level0 = 0)
    ...
    for h in new_hubs:
        ...
        if not caps:
            continue
        L_h[h] = {0: 0.0}
        # if caps[0] is 0, skip it, assign rest from level 1
        tail = caps[1:] if len(caps) > 1 and caps[0] == 0 else caps[1:]
        # if your data always [0, 5000, 8000, 10000] then tail = [5000,8000,10000]
    ...
    # IMPORTANT: If caps[0]=0 (format "0;500000;...") use first positive level
    # to avoid u_hub <= 0 when flow through hub -> infeasible
    for h in H0:
        caps = node_capacity_pcu_levels.get(h, [])
        if not caps:
            continue
        ...
        if cap0 <= 0:
            cap0 = 1e9  # fallback: existing hub must have capacity to avoid infeasible
        L_h[h] = {0: cap0}
...
if _use_model_cache:
    ...
    L_a = _cache['L_a']
    w_gk = _cache['w_gk']
    print("  ✓ Loaded L_h, L_a, w_gk from cache")
else:
    L_h = build_L_h(
        node_capacity_pcu_levels=node_capacity_pcu_levels,
        new_hubs=new_hubs,
    ...
    # Capacity for upgradable arcs (potential arcs) - theo Eps_02
    print(f"\n  c) CÁC CẠNH CÓ THỂ NÂNG CẤP (potential arcs):")

    L_a = {}
    for a in A_tilde:
        # Base capacity (level 0) - enough for basic demand
...
def build_through_hub_mapping_all(H, A_tilde):
    """
    General: create through_hub_mapping for all (hub -> node) pairs and all modes.
    
    Mapping: (f"{h}{m}", f"{j}{m}") -> (h, f"{j}{m}")
    
    Inputs:
      - H: list hubs (e.g. [3,4,6])
      - nodes: list of physical nodes hub can reach
               if None -> use union(H) (not recommended) 
               => pass list of physical nodes (e.g. N_real)
      - modes: tuple/list of mode suffixes (vd ('^1','^2'))
...
print("\n[4] Set upgrade cost (per unit capacity):")

f_la = {}

# Upgrade cost for real arcs
...
# Through-hub arcs have upgrade cost same as real arc
print("\n  b) Upgrade cost for through-hub arcs (same as real arc):")
...
for thru_arc, real_arc in through_hub_mapping.items():
    ...
    print(f"     Through-hub arc {thru_arc} ({mode}): {cost} (same as {real_arc})")

# Virtual arcs have NO upgrade cost (cannot upgrade)
print("\n  c) Upgrade cost for virtual arcs = 0:")
# All virtual arcs (mode node → physical node)
...
# 3) ADD: Base cost for reverse arcs of real arcs
print("\n[2.5] Add base cost for reverse arcs:")
for arc in real_arcs:
    # Create reverse arc: (j^m, i) from (i, j^m)
    j_physical = physical_node(arc[1])  # Get physical node from j^m
    reverse_arc = (arc[1], j_physical)  # (j^m, j)
    
    # Assign same base cost for reverse arc
    base_costs[reverse_arc] = BASE_CONST
    print(f"  Added: {reverse_arc}: {BASE_CONST}")

# 4) Assign real arc costs (apply discount if hub-hub)
print("\n[3] Real arc cost (into hub):")
for arc in real_arcs:
    if arc in base_costs:
        ...
        # Discount condition: both i and j are hubs in H
        if (i in H) and (j in H):
            cost = base * HUB_HUB_DISCOUNT
            note = "GIẢM 25% (hub-hub)"
        else:
            cost = base
            note = "KEEP (not hub-hub)"
        
        c_a[arc] = cost
        mode = 'road' if '^1' in arc[1] else 'waterway'
        print(f"  {arc} ({mode}): {cost:.2f} | base={base} | {note}")

# 5) ADD: Cost for reverse arcs (must compute like real arcs)
print("\n[4] Cost for reverse arcs:")
for arc in real_arcs:
    ...
    if reverse_arc in base_costs:
        ...
        i_rev = physical_node(reverse_arc[0])  # from j^m get j
        j_rev = physical_node(reverse_arc[1])  # j
        
        # Discount condition: both ends are hubs
...
# 6) Assign through-hub arc costs (NO discount)
print("\n[5] Through-hub arc cost (not into hub):")
for arc in through_hub_arc:
    if arc in base_costs:
        cost = base_costs[arc]
        c_a[arc] = cost
...
print("\n[6] Virtual arc cost (always = 0):")
for arc in virtual_arcs_all:
    c_a.setdefault(arc, 0.0)
    print(f"  {arc}: 0.0")

# 8) CHECK SYMMETRY
print("\n[7] Check cost symmetry:")
for arc in real_arcs:
    j_physical = physical_node(arc[1])
    reverse_arc = (arc[1], j_physical)
...
# Create c_h dict for ALL hubs in H with default value
c_h = {h: 1000 for h in H}

# Define budget per period (theo Data Model _ 2026-Feb_05-Eps_02.ipynb)
B = {
...
print(f"  Service cost c_h (for all {len(H)} hubs): all = 1000")
print(f"  Budget B theo period: {B}")


# ------------------------------------------------
# 1) HUB CAPACITY DICTIONARY L_h
#    Created by build_L_h() above (theo Eps_02 + fix caps[0]=0 for existing hubs)
#    DO NOT overwrite L_h - keep build_L_h result
# ------------------------------------------------
print("\n[1] THIẾT LẬP CAPACITY CHO HUBS (theo Data Model _ 2026-Feb_05-Eps_02.ipynb):")

# a) NEW HUBS (must open to have capacity)
print("\n  a) NEW HUBS (must open to have capacity):")
if len(new_hubs) == 0:
    print("    (None)")
else:
        ...
        if h in L_h:
            ...
            for l in sorted(L_h[h].keys()):
                ...
                if l == 0:
                    ...
                    print(f"      Level {l}: {cap:,.0f} (upgrade cost: {up_cost} per unit)")
            print("      Note: If hub at level 0: cannot use hub-service, must use through-hub paths")

# b) POTENTIAL HUBS (can upgrade, level 0 is base capacity)
print("\n  b) POTENTIAL HUBS (can upgrade):")
if len(H_tilde) == 0:
    print("    (None)")
else:
...
# c) EXISTING HUBS (fixed, cannot upgrade)
print("\n  c) EXISTING HUBS (cannot upgrade):")
if len(H0) == 0:
    print("    (None)")
else:
    for h in sorted(H0):
        if h in L_h:
            ...
            print(f"    Hub {h}:")
            print(f"      Level 0: {cap0:,.0f} (fixed, cannot upgrade)")

print(f"\n✓ Hub classification:")
print(f"  - New hubs (must open): {sorted(new_hubs)}")
print(f"  - Potential hubs: {sorted(H_tilde)}")
print(f"  - Existing hubs: {sorted(H0)}")


# ============================================
# 2) IDENTIFY ALL ARC TYPES
# ============================================
print("\n[2] Identify all arc types:")

print(f"  • Virtual arcs: {len(virtual_arcs_all)} arcs")
if len(virtual_arcs_all) <= 10:
    for arc in list(virtual_arcs_all)[:10]:
        print(f"      {arc}")
    if len(virtual_arcs_all) > 10:
        print(f"      ... and {len(virtual_arcs_all)-10} more arcs")

print(f"  • Potential arcs: {len(A_tilde)} arcs")
if len(A_tilde) <= 10:
    for arc in list(A_tilde)[:10]:
        print(f"      {arc}")
    if len(A_tilde) > 10:
        print(f"      ... and {len(A_tilde)-10} more arcs")

print(f"  • Existing arcs: {len(A0)} arcs")
if len(A0) <= 10:
    for arc in list(A0)[:10]:
        print(f"      {arc}")
    if len(A0) > 10:
        print(f"      ... and {len(A0)-10} more arcs")

# Capacity for virtual arcs = VERY LARGE
M = 10_000_000  # Very large number representing infinity
# Capacity for existing arcs (cannot upgrade) - GIỮ LẠI 10,000,000
existing_arc_capacity = 10_000_000

print(f"\n  Default capacity:")
print(f"    - Virtual arcs: {M:,} (very large)")
print(f"    - Existing arcs: {existing_arc_capacity:,} (fixed)")


# ============================================
# 3) HÀM LẤY HUB CAPACITY
# ============================================
print("\n[3] Get hub capacity function:")

def get_hub_capacity(hub, upgrade_level=0):
    """
    Get hub capacity based on hub type and upgrade level
    """
    if hub not in L_h:
        return 0.0
    
    # Existing hub: only level 0, cannot upgrade
    if hub in H0:
        if upgrade_level == 0:
            return L_h[hub][0]
        else:
            # Warning but return capacity level 0
            print(f"  [WARNING] Hub {hub} is existing hub, cannot upgrade. Return capacity level 0.")
            return L_h[hub][0]
    
    # Potential/New hub: can upgrade
    if upgrade_level in L_h[hub]:
        return L_h[hub][upgrade_level]
    else:
        # If level not found, return max available level
...
# ============================================
print("\n[4] Unified capacity function for all arc types:")

def get_arc_capacity(arc, level=0):
    """
    Get capacity of an arc
    """
    # 1. Virtual arcs: capacity very large
    if arc in virtual_arcs_all:
        return M
    
    # 2. Potential arcs: can upgrade
    elif arc in A_tilde:
        if level == 0:
    ...
    # 3. Existing arcs: fixed capacity (10,000,000)
    elif arc in A0:
        return existing_arc_capacity
    
    # 4. Through-hub arcs: special handling
    elif arc in through_hub_arc:
        # Find corresponding real arc
        real_arc = None
        ...
        if real_arc and real_arc in L_a:
            ...
            return existing_arc_capacity  # If real arc is existing
        else:
...
print(f"✓ Virtual arcs: capacity = {M:,} (very large)")
print(f"✓ Potential arcs: can upgrade from level 0 to level 1")
print(f"✓ Existing arcs: fixed capacity = {existing_arc_capacity:,}")
...
# When real arc upgraded, corresponding through-hub arc also upgraded
upgrade_dependencies = through_hub_mapping

print(f"Have {len(upgrade_dependencies)} upgrade dependencies:")
shown = 0
...
# Base demand and growth_factor - per Eps_02
BASE_DEMAND = 10_000
growth_factor = {1: 1.0, 2: 1.5, 3: 2.0}
T_list = list(T)

if not _use_model_cache:
    # Create w_gk from OD_pairs (per Eps_02)
    w_gk = {}
    for g, od_list in OD_pairs.items():
    ...
    # Save cache so no need to rerun
    ...
    print(f"  ✓ Saved cache: {_model_cache_path}")

print(f"  ✓ Created w_gk cho {len(OD_pairs)} commodities:")
print(f"  ✓ Base demand per commodity: {BASE_DEMAND:,}")
for g in OD_pairs:
    od_list = OD_pairs[g]
    commodity_name = "Passenger" if g == 'g1' else "Rice" if g == 'g2' else g
    print(f"\n    • {commodity_name} ({g}): {len(od_list)} OD pairs")
    
    # Total demand for this commodity
    total_demand_g = 0
    for od in od_list:
        for t in T_list:
            if (g, od, t) in w_gk:
    ...
    print(f"      Total demand (all periods): {total_demand_g:,.0f}")
    
    # Show first 2 ODs and first 2 periods
    for od in od_list[:2]:
        print(f"      OD {od[0]}→{od[1]}:")
        for t in T_list[:2]:
            if (g, od, t) in w_gk:
                print(f"        Period {t}: {w_gk[(g, od, t)]:,.0f}")
    
    if len(od_list) > 2:
        print(f"      ... and {len(od_list)-2} more OD pairs")


# ============================================
# 7) KIỂM TRA TÍNH KHẢ THI VỚI DỮ LIỆU LỚN
# ============================================
print("\n[6] Check model feasibility (with large data):")
...
# Total demand all periods
total_all_periods = sum(w_gk.values())
print(f"  Total demand all periods: {total_all_periods:,.0f}")

print("\n  Check capacity at important hubs:")
important_hubs = []
if new_hubs: important_hubs.extend(new_hubs[:3])  # Take first 3 new hubs
if H_tilde: important_hubs.extend(H_tilde[:3])    # Take first 3 potential hubs
if H0: important_hubs.extend(H0[:3])              # Take first 3 existing hubs

# Take period with max demand
max_t = max(T_list)
...
for hub in important_hubs:
    if hub in L_h:
        if hub in H0:
            ...
            hub_type = "Existing hub (fixed)"
        else:
            # Take max capacity level
            max_level = max(L_h[hub].keys())
            hub_cap = get_hub_capacity(hub, max_level)
            hub_type = f"Non-existing hub (max level {max_level})"
        
        print(f"\n  Hub {hub} ({hub_type}):")
        print(f"    - Max capacity: {hub_cap:,.0f}")
        
        # Estimated demand through this hub (allocated proportionally)
        # Assume main hubs handle most demand
        if hub in [6, 10, 12] or hub in new_hubs[:3] or hub in H_tilde[:3]:
            # Important hub: handles more demand
            hub_demand = total_demand_max_t * 0.7  # 70% demand
        else:
            # Secondary hub: handles less demand
            hub_demand = total_demand_max_t * 0.3  # 30% demand
        
        print(f"    - Estimated demand (period {max_t}): {hub_demand:,.0f}")
        
        if hub_demand <= hub_cap:
            print(f"    ✓ ENOUGH capacity: {hub_demand:,.0f} ≤ {hub_cap:,.0f}")
            print(f"      Surplus capacity: {hub_cap - hub_demand:,.0f} ({((hub_cap - hub_demand)/hub_cap*100):.1f}%)")
        else:
            print(f"    ✗ INSUFFICIENT capacity: {hub_demand:,.0f} > {hub_cap:,.0f}")
            print(f"      Shortfall: {hub_demand - hub_cap:,.0f} (need to upgrade {((hub_demand - hub_cap)/hub_cap*100):.1f}%)")


# ============================================
# 8) KIỂM TRA HÀM GET_HUB_CAPACITY
# ============================================
print("\n[7] Check get_hub_capacity function:")
    ...
    if hub in L_h:
        ...
        for level in levels_to_check:
            ...
            if abs(capacity - actual) < 0.01:
                ...
                print(f"    ✗ Level {level}: {capacity:,.0f} (actual: {actual:,.0f})")


print("\n=== KẾT LUẬN (VỚI DỮ LIỆU LỚN) ===")
print("✓ c_h: all hubs have service cost = 1")
print(f"✓ L_h: hub capacity (can reach millions)")
print(f"✓ Default capacity: existing arcs = {existing_arc_capacity:,}, virtual arcs = {M:,}")
print(f"✓ Base demand: {BASE_DEMAND:,} per commodity")
print(f"✓ Total demand all periods: {total_all_periods:,.0f}")
print("✓ Model scaled for large real-world data")
...
print("\n" + "="*60)
print("INITIALIZE MODEL AND DECISION VARIABLES")
print("="*60)
...
y_h = {}
print("\n[1] Hub state variables (y_h):")
print("  a) NEW HUB 3 (4 levels, level 0 = not open):")
for h in new_hubs:  # [3]
    for l in [0, 1, 2, 3]:  # 4 levels, level 0 = not open
        for t in T:
...
print("\n  c) Existing hub (6) - only level 0:")
for h in H0:
    for t in T:
        # Hub 6 always at level 0
        var_name = f"y_hub_{h}_l0_t{t}"
        y_h[(h, 0, t)] = model.addVar(vtype=GRB.BINARY, name=var_name)
        print(f"    {var_name}")
    
        # Create level 1, 2 vars for hub 6 (will be constrained = 0)
        for l in [1, 2]:
            var_name = f"y_hub_{h}_l{l}_t{t}"
            y_h[(h, l, t)] = model.addVar(vtype=GRB.BINARY, name=var_name)
            print(f"    {var_name} (will be constrained = 0)")


# 3.2 Arc state variables
y_a = {}
print("\n[2] Arc state variables (y_a):")
for a in A_tilde:
    for l in [0, 1]:
        for t in T:
...
v_path = {}
print("\n[3] Flow allocation variables (v_path):")
...
u_hub = {}
print("\n[4] Total flow through hub variables (u_hub):")
for h in H:  # [3, 4, 6]
    for t in T:
...
model.update()
print(f"\n✓ Created {model.numVars} variables")
print(f"  - Potential hubs (3,4): 3 levels × 2 periods × 2 hubs = 12 variables")
print(f"  - Existing hub (6): 3 levels × 2 periods = 6 variables (level 1,2 will be constrained = 0)")
print(f"  - Tổng variables y_h: 18 variables")


def get_hub_capacity_constraint(hub, t):
    """
    Return capacity constraint expression for hub
    """
    if hub in H0:
        # Existing hub: fixed capacity
        return L_h[hub][0]
    else:
        # Potential hub: capacity depends on level
        return gp.quicksum(
...
print("\n[1] Hub service cost (hub service cost):")
service_cost = gp.quicksum(
    c_h[h] * u_hub[(h, t)]
    for h in H for t in T
    if h in c_h
)
print(f"  ✓ Computed hub service cost")
...
print("\n[2] Transportation cost (transportation cost):")
transport_cost = gp.quicksum(transport_terms)
print(f"  ✓ Computed transportation cost")


# 8.3 Mode switching cost
print("\n[3] Mode switching cost (mode switching cost):")
# Cần tạo variables mode switching
mode_switch_terms = []
print("  Tạo variables mode switching và tính chi phí...")
for g, od_pairs_list in OD_pairs.items():
    for od in od_pairs_list:
        for idx, path in enumerate(paths[(g, od)]):
            for t in T:
                # Tạo variables cho mỗi hub trong path nếu có chuyển đổi mode
                for i in range(len(path) - 2):
                    current_arc = path[i]
                    next_arc = path[i+1]
                    
                    # Check if mode switch at hub
...
# 8.4 Total cost will be set sau khi có hub_upgrade_cost và arc_upgrade_cost (bên dưới)



# 8.4 Hub upgrade cost - SỬA LẠI CHO NEW HUB 3
print("\n[4] Hub upgrade cost (NEW HUB 3):")
hub_upgrade_cost = 0
hub_upgrade_vars = {}  # Biến mới để lưu trữ các variables nâng cấp

# a) NEW HUB 3 (has 4 levels: 0,1,2,3)
print("\n  a) NEW HUB 3 (4 levels, level 0 = not open):")
for h in new_hubs:  # [3]
    for t in T:
        if t == 1:
            # Period 1: Cost if not at level 0
            for l in [1, 2, 3]:
                if l in L_h[h]:
                    ...
                    print(f"  [WARNING] Hub {h} does not have level {l} in L_h")
        else:
            # Period t: Upgrade cost from period t-1
            for l in [1, 2, 3]:
                for prev_l in [0, 1, 2]:
                    if l > prev_l:  # Only compute when upgrading
                        capacity_increase = L_h[h][l] - L_h[h][prev_l]
                        cost_per_unit = f_lh.get(l, 0)
                        
                        # Tạo variables indicator cho nâng cấp
                        ...
                        # Constraint: upgrade_indicator = 1 if y_{h,l}^t = 1 và y_{h,prev_l}^{t-1} = 1
...
print(f"  - Hub 3: Upgrade cost computed for levels 1,2,3")

# b) POTENTIAL HUB 4 (has 3 levels: 0,1,2)
print("\n  b) POTENTIAL HUB 4 (3 levels):")
for h in H_tilde:  # [4]
    for t in T:
        if t == 1:
...
print(f"  - Hub 4: Upgrade cost computed for levels 1,2")

# c) EXISTING HUB 7 - no upgrade cost
print("\n  c) EXISTING HUB 7: No upgrade cost (fixed level 0)")
print(f"  ✓ Total: {len([k for k in hub_upgrade_vars.keys() if k[0] in new_hubs])} variables upgrade cho hub 3")
print(f"                {len([k for k in hub_upgrade_vars.keys() if k[0] in H_tilde])} variables upgrade cho hub 4")

# 8.5 Arc upgrade cost
print("\n[5] Arc upgrade cost (arc upgrade cost):")
arc_upgrade_cost = 0
arc_upgrade_vars = {}  # Biến lưu trữ các variables nâng cấp arc

print("  Compute upgrade cost for potential arcs...")
for a in A_tilde:
    for t in T:
        if t == 1:
            # Period 1: Compute cost if at level 1
            ...
            # Period t: Compute upgrade cost from level 0 to 1
            capacity_increase = L_a[a][1] - L_a[a][0]
            cost_per_unit = f_la.get(a, 0)
            
            # Tạo variables indicator cho nâng cấp arc
            ...
            # Constraint: arc_upgrade_indicator = 1 if y_arc_level1^t = 1 và y_arc_level0^{t-1} = 1
...
print(f"  ✓ Computed upgrade cost cho {len(A_tilde)} potential arcs")

# 8.6 Hàm mục tiêu: ONLY operating cost (theo notebook – investment constrained by budget B[t])
print("\n[6] Tổng chi phí (total cost) – chỉ operating:")
total_cost = service_cost + transport_cost + mode_switch_cost
model.setObjective(total_cost, GRB.MINIMIZE)
print("✓ Set hàm mục tiêu: Minimize operating cost (service + transport + mode_switch)")
print(f"  - Hub/Arc upgrade NOT in objective – ràng buộc bởi budget B[t] từng period")
...
for g, od_pairs_list in OD_pairs.items():
    for od in od_pairs_list:
        for t in T:
            ...
            path_flow_count += 1
print(f"  ✓ Added {path_flow_count} ràng buộc path flow balance")

# 9.1b Flow through hub: u_hub[(h,t)] >= tổng demand đi qua hub h trong period t (so hub service cost reflects correctly)
def path_uses_hub(path, h):
    """True if path goes through hub h (node h appears in path dưới dạng h hoặc 'h^1', 'h^2', ...)."""
    for (u, v) in path:
...
print(f"  ✓ Added u_hub = flow through hub constraints")

# 9.1c Hub capacity constraint (must upgrade when flow exceeds capacity – theo notebook)
...
print(f"  ✓ Added hub capacity constraints (new / potential / existing)")

# 9.1d Define x_arc = flow on arc và arc capacity (potential arcs) – theo notebook
...
print("\n[1d] Define x_arc = flow on arc và arc capacity:")
for a in A:
    for t in T:
...
print(f"  ✓ Added x_arc definition and capacity for potential arcs")

# 9.2 Hub level: each hub must select exactly 1 level mỗi period
...
print(f"  ✓ Added single level constraints for hubs")

# 9.3 Arc level: each arc must select exactly 1 level mỗi period
...
print(f"  ✓ Added single level constraints for arcs")

# 9.4 Budget: investment_cost_per_period[t] == (hub + arc investment trong t), and <= B[t] (theo notebook)
print("\n[4] Budget constraint per period (investment ≤ B[t]):")
...
print(f"  ✓ Added ràng buộc budget cho từng period")
...
if model.status in [GRB.OPTIMAL, GRB.TIME_LIMIT]:
    ...
    # Tính các thành phần chi phí từ variables
    print(f"\n{'='*55}")
   ...
   # Transportation cost
    ...
    # a) NEW HUB 3
    print("\n  a) NEW HUB 3 (must open to have capacity):")
    for h in new_hubs:  # [3]
        for t in T:
            hub_level = None
            for l in [0, 1, 2, 3]:
    ...
    for h in H_tilde:
        ...
        for t in T:
            if t == 1:
                ...
                # Period t: Tính upgrade cost từ period t-1
                for l in [1, 2]:
                    for prev_l in [0, 1]:
                        if l > prev_l:  # Only compute when upgrading
                            # Tìm variables upgrade_indicator
                            var_name = f"hub_upgrade_{h}_{prev_l}_to_{l}_t{t}"
    ...
    for a in A_tilde:
        ...
        for t in T:
            if t == 1:
                ...
                # Period t: Tìm variables arc_upgrade_indicator
    ...
    if diff > 1:
        print(f"\n  ⚠ Lưu ý: Have chênh lệch {diff:.2f} giữa objective và tổng chi phí tính toán")
    else:
    ...
    # Existing hub (6)
    print("\n  b) Existing hub (fixed):")
    for t in T:
        if y_h[(7, 0, t)].X > 0.5:
            capacity = L_h[7][0]
            print(f"    Hub 7 period {t} → level 0 (EXISTING, FIXED)")
            print(f"      Capacity: {capacity:,} (fixed)")
        else:
    ...
    if capacity_violations == 0:
        ...
        print(f"\n  ⚠ Have {capacity_violations} capacity constraint violations!")
    ...
    def export_results_to_json(period: int = 1, output_dir: str = "data/Mekong"):
        ...
        for h in H_tilde:
            if period == 1:
                ...
                # Period > 1: Tính upgrade cost từ period trước
                for l in [1, 2]:
                    for prev_l in [0, 1]:
                        if l > prev_l:
